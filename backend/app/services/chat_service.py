"""Chat service using repositories and database with vector search."""
import uuid
...
class ChatService:
    """Chat service using repositories and async database with vector search."""
    ...
    async def _fallback_search(
        ...
        result = await dishes_repo.search_with_country_priority(
            query_embedding,
            country,
            query_text=dish_name,  # Pass query text for exact/prefix matching
            threshold=0.70,  # Higher threshold even in fallback mode
            min_confidence=0.75
        )
        ...
        # Try as single ingredient in USDA with fallback
        logger.info(f"Trying USDA search for '{dish_name}'...")
        usda_result = await usda_repo.search_with_fallback(
            query_text=dish_name,
            query_embedding=query_embedding,
            threshold=0.5  # Lower threshold for fallback mode
        )
    ...
    async def _process_single_ingredient(
        ...
        result = await usda_repo.search_with_fallback(
            query_text=ingredient_base.name,
            query_embedding=query_embedding,
    ...
    async def _process_dish(
        ...
        result = await dishes_repo.search_with_country_priority(
            query_embedding,
            country,
            query_text=gpt_response.dish_name,  # Pass query text for exact/prefix matching
            threshold=0.65,  # Higher threshold to reduce false positives
            min_confidence=0.65  # Higher minimum confidence for good matches
        )
        ...
        if result:
            ...
            # Double-check it's not a different dish (additional safety check)
            query_lower = gpt_response.dish_name.lower().strip()
            dish_name_lower = dish.dish_name.lower().strip()
            
            # Check word overlap - if no common meaningful words, reject
            ...
            if not common_words and len(query_words) > 0 and len(dish_words) > 0:
                # No common words - likely different dishes
                logger.warning(
                    f"Rejecting match: '{gpt_response.dish_name}' -> '{dish.dish_name}' "
                    f"- No common words, likely different dishes. Falling back to GPT breakdown..."
                )
                result = None
            elif similarity < 0.65:
                logger.warning(
                    f"Low similarity match: '{gpt_response.dish_name}' -> '{dish.dish_name}' "
                    f"(similarity: {similarity:.3f}). Falling back to GPT breakdown..."
                )
                result = None  # Force GPT breakdown path
            
            if result:  # Only use database result if similarity is acceptable
        ...
        if not result:  # Either no match or similarity too low
            ...
            for ing_base in gpt_response.ingredients_breakdown:
                ...
                ing_result = await usda_repo.search_with_fallback(
                    query_text=ing_base.name,
                    query_embedding=ing_embedding,
                    threshold=0.5  # More permissive threshold
                )
        ...
        if gpt_response.modifications:
            ...
            try:
                ingredients = await recipe_modifier.apply_modifications(
                    ingredients,
                    gpt_response.modifications,
                    db
                )
                logger.info(f"Modifications applied successfully. Final ingredient count: {len(ingredients)}")
            except Exception as e:
